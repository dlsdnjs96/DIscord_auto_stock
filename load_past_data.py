...
class Creon:
        ...
        if not self.connected():
            ...
            app.start(
                'C:\CREON\STARTER\coStarter.exe /prj:cp /id:{} /pwd:{} /pwdcert:{} /autostart'.format(
                    userInfo.getId(), userInfo.getPw(), userInfo.getPwcert()
                )
            )
    ...
    def get_code_list(self, market):
        ...
        code_list = ['A005930', 'A035420', 'A000660', 'A051915', 'A035720', 'A005380']
        return code_list
...
def download_data(term):
    for i in range(len(creon.sv_code_df)):
        ...
        stock_data = creon.down_chart(creon.sv_code_df['종목코드'][i], term, creon.fromDate, creon.toDate)
        ...
        if (lastDay != None):
            ...
            if(last_day_data['종가'][0] == lastDay[1]):
                ...
                stock_data = creon.down_chart(creon.sv_code_df['종목코드'][i], term, START_DATE, creon.toDate)

        if stock_data.empty:
            print(creon.sv_code_df['종목코드'][i], creon.sv_code_df['종목명'][i], ' 데이터 조회결과 없어서 넘김')
            continue

        print(creon.sv_code_df['종목코드'][i], creon.sv_code_df['종목명'][i], ' 데이터 넣는중', datetime.now())
        stock_data.to_csv('C:\\Users\\dlsdn\\source\\repos\\Auto_Stocks_Trading\\stock_data_{}.csv'.format(term), encoding='ms949')
        print(stock_data)
...
if __name__ == '__main__':
    ...
    if weekDay[week] == 'Mon' or weekDay[week] =='Tue' or weekDay[week] =="Wed" or weekDay[week] =='Thu' or weekDay[week] =='Fri':
        if int(now.strftime('%H')) >= 6 and int(now.strftime('%H')) <= 16:
            print('다운 가능 시간이 아닙니다 {}'.format(int(now.strftime('%H'))))
            sys.exit()
...
for i in range(len(creon.sv_code_df)):
    ...
    cursor_1m.execute(
        ...
        from [STOCK1m_3].dbo.+tablename+ l1 join (
            ...
            (
                select 종목코드, 날짜, 시가
                , (SELECT TOP (1) 시가 FROM  [STOCK1m_3].dbo. WHERE 종목코드 = d.종목코드 AND 날짜 = d.날짜 ORDER BY 시간 ) 분봉시가
